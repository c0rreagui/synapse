...
async def sync():
    async with httpx.AsyncClient(timeout=15) as c:
        ...
        for t in targets:
            ...
            if info:
                t.profile_image_url = info.get("profile_image_url", "")
                t.offline_image_url = info.get("offline_image_url", "")
                print(f"Updated {t.channel_name} with profile: {info.get('profile_image_url')} and offline: {info.get('offline_image_url')}")
        db.commit()
