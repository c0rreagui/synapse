...
target_time = "2026-02-15 00:45:00"

with engine.connect() as conn:
    # Get current item info
    result = conn.execute(text("SELECT id, profile_id, status, video_path, scheduled_time FROM scheduled_items WHERE id = 31"))
    item = result.fetchone()
    
    if item:
        print(f"Item 31 Current State:")
        print(f"  profile_id: {item[1]}")
        print(f"  Status: {item[2]}")
        print(f"  video_path: {item[3]}")
        print(f"  scheduled_time: {item[4]}")
        
        # Update item  - change profile_id to 2 (Opinião Viral - Active)
        conn.execute(text("""
        ...
        print(f"\nItem 31 Updated:")
        print(f"  profile_id: 2 (Opinião Viral)")
