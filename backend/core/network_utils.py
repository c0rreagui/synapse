...
# --- Dynamic Chrome Version Detection (shared with browser.py) ---
def _detect_chrome_version() -> str:
    """Detect real Chromium version from binary, with fallback."""
    paths = ["/usr/bin/chromium", "/usr/bin/google-chrome", "/usr/bin/chromium-browser"]
    for path in paths:
        if os.path.isfile(path):
            try:
                ...
                pass
    return "131.0.0.0"  # Modern fallback


# Detect Chrome version independently (no circular import with browser.py)
CHROME_VERSION = _detect_chrome_version()
CHROME_MAJOR = CHROME_VERSION.split(".")[0]
DEFAULT_UA = f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{CHROME_VERSION} Safari/537.36"
